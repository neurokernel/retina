...
class Photoreceptor(BaseNeuron):
    ...
    def _setup_output(self):
        ...
        if self.record_microvilli:
            ...
            self.outputfile_dra = tables.openFile(outputfile+'dra.h5', 'w')
            self.outputfile_dra.createEArray(
                "/", "array",
                tables.Float64Atom() if self.dtype == np.double else tables.Float32Atom(),
                (0, self.num_neurons))

            self.outputfile_dri = tables.openFile(outputfile+'dri.h5', 'w')
            self.outputfile_dri.createEArray(
                "/", "array",
                tables.Float64Atom() if self.dtype == np.double else tables.Float32Atom(),
                (0, self.num_neurons))

            self.outputfile_sa = tables.openFile(outputfile+'sa.h5', 'w')
            self.outputfile_sa.createEArray(
                "/", "array",
                tables.Float64Atom() if self.dtype == np.double else tables.Float32Atom(),
                (0, self.num_neurons))

            self.outputfile_si = tables.openFile(outputfile+'si.h5', 'w')
            self.outputfile_si.createEArray(
                "/", "array",
                tables.Float64Atom() if self.dtype == np.double else tables.Float32Atom(),
                (0, self.num_neurons))

            self.outputfile_nov = tables.openFile(outputfile+'nov.h5', 'w')
            self.outputfile_nov.createEArray(
                "/", "array",
                tables.Float64Atom() if self.dtype == np.double else tables.Float32Atom(),
                (0, self.num_neurons))
    ...
    def _setup_update_state(self, n_dict):
        self.I_fb = garray.empty(self.num_neurons, self.dtype)
    ...
    def post_run(self):
        ...
        if self.record_microvilli:
            ...
            self.outputfile_dra.close()
            self.outputfile_dri.close()
            self.outputfile_sa.close()
            self.outputfile_si.close()
            self.outputfile_nov.close()
    ...
    def _write_outputfile(self):
        ...
        if self.record_microvilli:
            ...
            self.outputfile_X0.root.array.append(
                [np.sum(tmp[:, 0::2], axis=1)])
            self.outputfile_X0.flush()
            self.outputfile_X1.root.array.append(
                [np.sum(tmp[:, 1::2], axis=1)])
            self.outputfile_X1.flush()

            tmp = self.X[1].get().view(np.int16)  # PLC* D*
            self.outputfile_X2.root.array.append(
                [np.sum(tmp[:, 0::2], axis=1)])
            self.outputfile_X2.flush()
            self.outputfile_X3.root.array.append(
                [np.sum(tmp[:, 1::2], axis=1)])
            self.outputfile_X3.flush()

            tmp = self.X[2].get().view(np.int16)  # C* T*
            self.outputfile_X4.root.array.append(
                [np.sum(tmp[:, 0::2], axis=1)])
            self.outputfile_X4.flush()
            self.outputfile_X5.root.array.append(
                [np.sum(tmp[:, 1::2], axis=1)])
            self.outputfile_X5.flush()

            self.outputfile_X6.root.array.append(
                [np.sum(self.X[3].get(), axis=1)])  # M*
            self.outputfile_X6.flush()

            self.outputfile_sa.root.array.append(self.hhx[0].get())
            self.outputfile_sa.flush()

            self.outputfile_si.root.array.append(self.hhx[1].get())
            self.outputfile_si.flush()

            self.outputfile_dra.root.array.append(self.hhx[2].get())
            self.outputfile_dra.flush()

            self.outputfile_dri.root.array.append(self.hhx[3].get())
            self.outputfile_dri.flush()

            self.outputfile_nov.root.array.append(self.hhx[4].get())
            self.outputfile_nov.flush()
...
#            self.randState.gpudata, self.X[3].gpudata, self.num_neurons,
#            self.num_microvilli, self.photons.gpudata)
             ...
             float dt, %(type)s* d_Vm, %(type)s* g_ns, %(type)s* input)
{
    ...
    if(tid == 0)
    {
        Vm = d_Vm[bid] * 0.001;  // V
        ns = g_ns[bid];
        lambda = input[bid]/(NUM_MICROVILLI*dt);
    }
