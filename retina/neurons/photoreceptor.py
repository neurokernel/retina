...
class Photoreceptor(BaseNeuron):
    ...
    def __init__(self, n_dict, V_p, input_dt, debug=False, LPU_id=None,
                 cuda_verbose = False):
        self.num_neurons = len(n_dict['id'])  # NOT n_dict['num_neurons']
        self.num_microvilli = np.asarray(n_dict['num_microvilli'], dtype = np.int32)
    ...
    def eval(self, st=None):
        for _ in range(self.multiple):
            ...
            self.sum_current_func.prepared_call(
                ...
                self.V_p, self.I.gpudata, self.I_fb.gpudata)
