...
class LPU(Module):
    ...
    def graph_to_dicts(graph):
        ...
        4. V_rev - If the `conductance` attribute is True, this attribute
    ...
    def run_step(self):
        ...
        if not self.first_step:
            self.log_debug('updating neurons state and output')
            for neuron in self.neurons:
                self.log_debug('updating {} state'.format(neuron.__class__.__name__))
                if self.has_synapse:
                    neuron.update_internal_state(self.synapse_state.gpudata)
                self.log_debug('evaluating {}'.format(neuron.__class__.__name__))
                neuron.eval()

            self.log_debug('updating buffer')
            self._update_buffer()

            self.log_debug('updating synapse state')
            for synapse in self.synapses:
    ...
    def _get_external_input(self):
        ...
        self.log_debug('input from generator function updated')
