...
class ReducedPhotoreceptorModel(BaseMembraneModel):
    accesses = ['photon', 'I']
    updates = ['V']
...
#define b1     200 /* x1 synthesis rate*/
#define d1     5 /* x1 diassocaite rate*/
#define b2     3. /* x2 synthesis rate*/
#define d2     4. /* x2 diasscociate rate*/
#define x1_max 1.
#define x2_max 1.
#define r  130 /* r is r */
#define a1 5.5 /* parameters shifting the sigmoid */
#define a2 1.5 /* parameters shifting the sigmoid */
...
{
    double  dx1_dt = b1 * compute_f(lmbd)  * (x1_max-x1) - (d1 + r * x2) * x1;
    return x1 + dx1_dt * dt;
}

__device__ double compute_x2 (double x1, double x2, float dt)
{
    double dx2_dt = b2 * x1 * (x2_max - x2) - d2 * x2;
    return x2 + dx2_dt * dt;
}
