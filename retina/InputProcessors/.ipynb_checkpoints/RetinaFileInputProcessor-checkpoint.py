...
class RetinaInputProcessor(BaseInputProcessor):
    def __init__(self, config, retina, input_video):
        self.for_test = 39
        self.config = config

        self.screen_type = config['Retina']['screentype']
        self.filtermethod = config['Retina']['filtermethod']
        screen_cls = cls_map.get_screen_cls(self.screen_type) #screen_cls here is SphereScreen in screen files
        self.screen = screen_cls(config) # SphereScreen 
        self.retina = retina
    ...
    def generate_datafiles(self):
        ...
        screen.setup_file('intensities.h5', read_file=False, read_video=True)
